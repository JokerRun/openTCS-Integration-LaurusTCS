...
public class ControlPanel
   ...
   */
  private LaurusTcsProcessModelTO processModel;
   ...
   */
  @Inject
  public ControlPanel(@Assisted LaurusTcsProcessModelTO processModel,
  ...
  private void initGuiContent() {
    ...
    for (LaurusTcsProcessModel.Attribute attribute : LaurusTcsProcessModel.Attribute.values()) {
      processModelChange(attribute.name(), processModel);
    }
  }
  ...
  public void processModelChange(String attributeChanged, VehicleProcessModelTO newProcessModel) {
    if (!(newProcessModel instanceof LaurusTcsProcessModelTO)) {
      return;
    }
    processModel = (LaurusTcsProcessModelTO) newProcessModel;
    ...
    else if (Objects.equals(attributeChanged, LaurusTcsProcessModel.Attribute.VEHICLE_HOST.name())) {
      updateVehicleHost(processModel.getVehicleHost());
    }
    else if (Objects.equals(attributeChanged, LaurusTcsProcessModel.Attribute.VEHICLE_PORT.name())) {
      updateVehiclePort(processModel.getVehiclePort());
    }
    else if (Objects.equals(attributeChanged,
                            LaurusTcsProcessModel.Attribute.VEHICLE_IDLE_TIMEOUT.name())) {
      updateVehicleTimeout(processModel.getVehicleIdleTimeout());
    }
    else if (Objects.equals(attributeChanged, LaurusTcsProcessModel.Attribute.VEHICLE_IDLE.name())) {
      updateVehicleIdle(processModel.isVehicleIdle());
    }
    else if (Objects.equals(attributeChanged,
                            LaurusTcsProcessModel.Attribute.DISCONNECTING_ON_IDLE.name())) {
      updateDisconnectingOnIdle(processModel.isDisconnectingOnVehicleIdle());
    }
    else if (Objects.equals(attributeChanged,
                            LaurusTcsProcessModel.Attribute.RECONNECTING_ON_CONNECTION_LOSS.name())) {
      updateReconnectingOnConnectionLoss(processModel.isReconnectingOnConnectionLoss());
    }
    else if (Objects.equals(attributeChanged, LaurusTcsProcessModel.Attribute.LAST_ORDER.name())) {
      updateLastOrder(processModel.getLastOrderSent(), processModel.isCommAdapterConnected());
    }
    else if (Objects.equals(attributeChanged,
                            LaurusTcsProcessModel.Attribute.LOGGING_ENABLED.name())) {
